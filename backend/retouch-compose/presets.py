...
Каждый пресет — набор параметров для всего pipeline композиции:
  ...
  - skin_rebuild: 0..1 — ПЕРЕСТРОЙКА тона кожи заново. В отличие от
      healing, не ищет дефекты по списку, а считает ровный тон по
      всей коже: всё, что от него отклоняется, исчезает само.
      Поэтому не бывает ни пропущенных прыщей, ни ореолов вокруг них.
  - skin_rebuild_radius: радиус оценки тона (доля кадра). Должен быть
      заметно больше прыща, иначе крупные очаги протекают в базу.
  - skin_rebuild_iters: число уточнений оценки (3-5)
  - skin_rebuild_keep: доля кожи, принимаемая за чистую (0.45-0.6)
  - skin_rebuild_detail: сколько своего микрорельефа оставить (0..1)
...
PRESETS: Dict[str, Dict[str, Any]] = {
    ...
    "medium": {
        ...
        "final_red_passes": 1,
        "micro_texture": 0.38,
        "skin_texture_amount": 2.2,
        "skin_texture_scale": 1.0,
        # Умеренная перестройка: ровнее тон, но рельеф лица сохраняется.
        "skin_rebuild": 0.55,
        "skin_rebuild_radius": 0.040,
        "skin_rebuild_iters": 3,
        "skin_rebuild_keep": 0.55,
        "skin_rebuild_detail": 0.45,
        "skin_rebuild_detail_clip": 7.0,
    ...
    "strong": {
        ...
        # на чистой щеке даёт std ≈ 13 единиц яркости.
        "skin_texture_amount": 3.0,
        "skin_texture_scale": 1.0,
        # ПЕРЕСТРОЙКА КОЖИ — главный шаг «Сильной». Тон кожи считается
        # заново по всей области, поэтому пропущенных дефектов не бывает
        # в принципе: прыщи, ямки и пятна просто не попадают в расчёт.
        "skin_rebuild": 0.92,
        # Радиус заметно больше прыща (4.5% кадра), иначе крупные очаги
        # протекают в базу и остаются видны.
        "skin_rebuild_radius": 0.045,
        "skin_rebuild_iters": 4,
        # Доля кожи, принимаемая за «чистую»: 0.45 = строго, отбрасываем
        # больше половины пикселей как потенциальные дефекты.
        "skin_rebuild_keep": 0.45,
        # Сколько собственного микрорельефа (поры, пушок) оставить поверх
        # перестроенного тона.
        "skin_rebuild_detail": 0.30,
        # Порог амплитуды микрорельефа: выше него это уже не поры,
        # а края дефектов — их обратно не возвращаем.
        "skin_rebuild_detail_clip": 4.0,
