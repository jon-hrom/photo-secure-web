"""Списание и возврат энергии за инструменты. Атомарно, через одну транзакцию."""
import os
import psycopg2
...
def refund(user_id: int, amount: int, description: str):
    """Возврат энергии при неудачной операции. Ошибки глушим — возврат не должен ломать ответ."""
    if amount <= 0:
        return
    conn = _conn()
    try:
