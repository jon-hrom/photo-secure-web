...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if method == 'POST':
        ...
        if action == 'auto_complete_by_folder':
            ...
            client_id = body_data.get('clientId') or body_data.get('client_id')
            if not user_id or not client_id:
                return {
                    'statusCode': 400,
                    'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                    'body': json.dumps({'error': 'userId and clientId required'}),
                    'isBase64Encoded': False
                }
            conn = psycopg2.connect(os.environ['DATABASE_URL'])
            try:
                with conn.cursor() as cur:
                    cur.execute(f'''
                        SELECT p.id, p.name
                        FROM {DB_SCHEMA}.client_projects p
                        JOIN {DB_SCHEMA}.clients c ON c.id = p.client_id
                    ...
                    project_id_db, project_name = row
                    cur.execute(
