...
def protect_mask_small(img: Image.Image) -> np.ndarray:
    ...
    # с прыщами стирало из защиты сами брови и глаза: их толщина меньше
    # ядра. Защита переставала работать там, ради чего она и нужна, и
    # генеративная модель безнаказанно перерисовывала взгляд и линию
    # брови. //60 (9 px) мелкие точки по-прежнему убирает, а черты лица
    # оставляет под защитой.
    open_k = _odd(max(5, min(img.size) // 60))
...
def detect_defects(arr: np.ndarray, skin: np.ndarray,
    ...
    sel = valid > 0.5
    # 3% площади кожи на «Лёгкой», 30% на «Сильной». Верхняя граница
    # поднята с 20%: на коже с россыпью акне в бюджет проходила лишь
    # половина реальных очагов, остальные оставались нетронутыми, и
    # «Сильная» на глаз почти не отличалась от «Стандарта».
    budget = 3.0 + 27.0 * sens
