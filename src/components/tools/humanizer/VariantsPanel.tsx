...
const VariantsPanel = ({ original, variants, loading, onApply, onRegenerate, onClose }: Props) => {
  return (
    <div className="p-3 sm:p-4 rounded-xl border bg-card space-y-3">
      <div className="flex items-start justify-between gap-2 sticky top-0 bg-card z-10 pb-2 border-b">
        <div className="flex-1 min-w-0">
          <div className="text-xs font-semibold text-muted-foreground mb-1">
            Оригинал предложения
          </div>
          <p className="text-sm italic leading-relaxed line-clamp-3">{original}</p>
        </div>
        ...
        {variants.map((v, i) => (
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm flex-1 leading-relaxed break-words">{v}</p>
            </div>
