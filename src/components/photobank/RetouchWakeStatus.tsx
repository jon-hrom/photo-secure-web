...
const RetouchWakeStatus = ({ waking, wakeStatus }: RetouchWakeStatusProps) => {
  ...
  if (waking) {
    return (
      <div className="rounded-xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/40 dark:border-amber-800 p-5 flex items-start gap-4 shadow-sm">
        <div className="relative flex-shrink-0 mt-0.5">
          <div className="absolute inset-0 rounded-full bg-amber-400/20 animate-ping" />
          <Icon name="Loader2" size={22} className="animate-spin text-amber-600 dark:text-amber-400 relative" />
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-sm font-medium text-amber-900 dark:text-amber-100">
            Сервис запускается для обработки фото, пожалуйста немного подождите
          </span>
  ...
  return (
    <div className="rounded-xl border border-red-200 bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-950/40 dark:to-rose-950/40 dark:border-red-800 p-4 flex items-center gap-3 shadow-sm">
      <Icon name="AlertCircle" size={20} className="text-red-600 dark:text-red-400 flex-shrink-0" />
      <span className="text-sm font-medium text-red-800 dark:text-red-200">{wakeStatus}</span>
    </div>
  );
};
