...
const RetouchWakeStatus = ({ waking, wakeStatus }: RetouchWakeStatusProps) => {
  ...
  if (waking) {
    ...
    const isWarmup = wakeStatus === 'warmup' || wakeStatus?.startsWith('warmup:');
    const warmupSec = wakeStatus?.startsWith('warmup:') ? parseInt(wakeStatus.split(':')[1], 10) : null;

    let title = 'Сервис запускается, пожалуйста подождите';
    let subtitle = 'Обычно это занимает 1–2 минуты';

    if (isWarmup) {
      title = 'Сервер запущен, прогреваем службы ретуши';
      subtitle = warmupSec !== null && !isNaN(warmupSec)
        ? `Осталось ~${warmupSec} сек., скоро начнём обработку`
        : 'Осталось совсем немного';
    } else if (isVmReady) {
      title = 'Сервер запущен, загружается сервис ретуши...';
